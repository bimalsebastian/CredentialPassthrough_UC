...
# Explicitly link submodule objects as attributes on the pyarrow module so that
# patch("pyarrow.parquet.read_table") can resolve via getattr() on all Python
# versions. Python <3.11 does not lazily expose pyarrow.parquet as an attribute
# without an explicit `import pyarrow.parquet`, causing patch() to raise
# AttributeError when the real (installed) pyarrow module is in sys.modules.
sys.modules["pyarrow"].parquet = sys.modules["pyarrow.parquet"]
sys.modules["pyarrow"].orc = sys.modules["pyarrow.orc"]
