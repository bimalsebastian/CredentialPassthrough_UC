...
class UCPassthroughFormatReader:
    ...
    def _load_via_adls_direct(self, path: str) -> DataFrame:
        ...
        dispatch = {
                ...
                'xlsx':       lambda: reader.read_xlsx_files(container, blob_path, options=opts),
                'audio':      lambda: reader.read_audio_files(container, blob_path, options=opts),
                'wav':        lambda: reader.read_audio_files(container, blob_path, options=opts),
                'mp3':        lambda: reader.read_audio_files(container, blob_path, options=opts),
